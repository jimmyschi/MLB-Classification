...
def train_multiple_iterations(X,y,experience_levels,iterations):
    ...
    for iter in range(iterations):
        for key, values in experience_levels.items():
            ...
            weights[key].append(w)         # w consists of 3 elements
            bias[key].append(b)      # b consists of 1 element
            num_classes = 7
            # y_pred_calc = np.zeros((X_test.shape[0], num_classes))
            # for i in range(X_test.shape[0]):
            #     y_pred_calc[i] = X_test[i,0]*w[0] + X_test[i,1]*w[1]  + X_test[i,2]*w[2]+ b
            #     # print("y_pred_calc[i]: " + str(y_pred_calc[i]))
            # y_pred = np.argmax(y_pred_calc,axis=1)
            ...
            if accuracy > best_accuracy[key]:
                ...
                best_weights[key] = svm_classifier.coef_
                best_bias[key] = svm_classifier.intercept_
...
train_multiple_iterations(X,y,experience_levels,10)